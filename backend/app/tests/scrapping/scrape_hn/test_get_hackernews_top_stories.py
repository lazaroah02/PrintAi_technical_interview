...
def test_get_hackernews_top_stories_success(
        mock_next, mock_extract,
        mock_load,
        mock_opts,
        mock_chrome
        ):
    ...
    mock_extract.return_value = [
        {"title": "Sample", "url": "https://x.com", "score": 100}
        ]
...
@patch(
    "app.scraping.scrape_hn.webdriver.Chrome",
    side_effect=WebDriverException("Driver failed")
    )
@patch("app.scraping.scrape_hn.build_chrome_options")
def test_get_hackernews_top_stories_webdriver_exception(
        mock_opts,
        mock_chrome
        ):
...
@patch("app.scraping.scrape_hn.build_chrome_options")
@patch(
    "app.scraping.scrape_hn.load_hn_page",
    side_effect=ConnectionError("No internet")
    )
def test_get_hackernews_top_stories_connection_error(
        mock_load,
        mock_opts,
        mock_chrome
        ):
...
def test_get_hackernews_top_stories_unexpected_error(
        mock_load,
        mock_opts,
        mock_chrome
        ):
