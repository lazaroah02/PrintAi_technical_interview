...
def test_get_hackernews_top_stories_success(mock_webdriver):
    ...
    mock_item.find_element.return_value.get_attribute.return_value = "http://example.com"
...
def test_get_hackernews_top_stories_connection_error(mock_webdriver):
    ...
    mock_webdriver.get.side_effect = Exception("Connection Error: Error reaching https://news.ycombinator.com/?p=1. Check the internet connection")

    with pytest.raises(Exception, match=r"Connection Error: Error reaching https://news.ycombinator.com/\?p=1\. Check the internet connection"):
