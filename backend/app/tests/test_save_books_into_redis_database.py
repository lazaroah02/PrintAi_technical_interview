...
books_data = [
    ...
    {
        "url": "https://books.toscrape.com/catalogue/book/2",
        "title": "Book 2",
        "price": "29.99"
    },
]
...
def test_save_books_into_redis_database_success(mock_logging_info, mock_redis):
    ...
    for book in books_data:
        ...
        mock_redis_instance.set.assert_any_call(
            redis_key, json.dumps(book, ensure_ascii=False)
        )

    # Check if the success logging is done
    mock_logging_info.assert_called_once_with(
        f"Stored {len(books_data)} books into Redis."
    )
...
def test_save_books_into_redis_database_redis_connection_error(
    mock_logging_error, mock_redis
):
    ...
    mock_logging_error.assert_called_once_with(
        "Failed to store data in Redis: Redis connection failed"
    )
...
def test_save_books_into_redis_database_empty_books(
    ...
    mock_logging_info.assert_called_once_with("Stored 0 books into Redis.")
...
@patch("redis.Redis")
@patch("app.redis_storage.logging.info")
def test_save_books_into_redis_database_logging_success(
    mock_logging_info, mock_redis
):
    # Mock Redis set method
    mock_redis_instance = MagicMock()
    mock_redis.return_value = mock_redis_instance
...
@patch("app.redis_storage.logging.error")
def test_save_books_into_redis_database_logging_failure(
    mock_logging_error, mock_redis
):
    with pytest.raises(redis.ConnectionError):
        save_books_into_redis_database(books_data)
    ...
    mock_logging_error.assert_called_once_with(
        "Failed to store data in Redis: Connection failed"
    )
