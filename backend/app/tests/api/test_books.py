...
def test_books_success(mock_redis, client):
    ...
    mock_r.get.return_value = json.dumps({
        "title": "Python 101", "category": "programming"
    }).encode('utf-8')
...
def test_books_search_filter(mock_redis, client):
    ...
    mock_r.get.side_effect = [
        json.dumps({"title": "Python for Beginners", "category": "programming"}).encode('utf-8'),
        json.dumps({"title": "Cooking Tips", "category": "cooking"}).encode('utf-8')
    ]
...
def test_books_pagination(mock_redis, client):
    ...
    mock_r.get.side_effect = [
        json.dumps({"title": f"Book {i}", "category": "fiction"}).encode('utf-8') for i in range(15)
    ]
